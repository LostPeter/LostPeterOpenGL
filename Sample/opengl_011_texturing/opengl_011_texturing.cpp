...
/////////////////////////// ModelMesh ///////////////////////////
bool OpenGL_011_Texturing::ModelMesh::LoadMesh(bool isFlipY, bool isTransformLocal, const FMatrix4& matTransformLocal)
{
    //1> Load
    FMeshData meshData;
    ...
    {
        F_LogError("*********************** OpenGL_011_Texturing::ModelMesh::LoadMesh: load mesh failed: [%s] !", this->pathMesh.c_str());
        return false; 
    }
    ...
    if (this->poTypeVertex == F_MeshVertex_Pos3Color4Normal3Tex2)
    {
        ...
        F_LogInfo("OpenGL_011_Texturing::ModelMesh::LoadMesh: load mesh [%s] success, [Pos3Color4Normal3Tex2]: Vertex count: [%d], Index count: [%d] !", 
                  this->nameMesh.c_str(),
    ...
    else if (this->poTypeVertex == F_MeshVertex_Pos3Color4Normal3Tangent3Tex2)
    {
        ...
        F_LogInfo("OpenGL_011_Texturing::ModelMesh::LoadMesh: load mesh [%s] success, [Pos3Color4Normal3Tangent3Tex2]: Vertex count: [%d], Index count: [%d] !", 
                  this->nameMesh.c_str(),
...
void OpenGL_011_Texturing::loadModel_Custom()
{
    ...
    for (int i = 0; i < g_ObjectCount; i++)
    {
        ...
        //Mesh
        {
            ...
            F_Assert(pMesh != nullptr && "OpenGL_011_Texturing::loadModel_Custom")
...
void OpenGL_011_Texturing::createShaderModules()
{
    for (int i = 0; i < g_ShaderCount; i++)
    {
        ...
        if (pShader == nullptr)
        {
            String msg = "*********************** OpenGL_011_Texturing::createShaderModules: Failed to create shader: shaderPath: " + shaderPath;
            F_LogError(msg.c_str());
            throw std::runtime_error(msg);
        }
        this->m_aGLShaderModules.push_back(pShader);
        this->m_mapGLShaderModules[shaderName] = pShader;
        F_LogInfo("OpenGL_011_Texturing::createShaderModules: create shader [%s] success !", shaderPath.c_str());
    }
}
...
void OpenGL_011_Texturing::destroyDescriptorSetLayouts()
{
...
void OpenGL_011_Texturing::createDescriptorSetLayouts()
{
...
DescriptorSetLayout* OpenGL_011_Texturing::findDescriptorSetLayout(const String& nameDescriptorSetLayout)
{
    ...
    for (size_t i = 0; i < count; i++)
    {
		...
		//Pipeline Graphics
		{
            ...
            for (int j = 0; j < count_ds; j++)
            {
                ...
                {
                    String msg = "*********************** OpenGL_011_Texturing::createDescriptorSets_Custom: Graphics: Wrong DescriptorSetLayout type: " + nameDescriptorSet;
                    F_LogError(msg.c_str());
                    throw std::runtime_error(msg.c_str());
                }
            }
...
void OpenGL_011_Texturing::cleanupSwapChain_Custom()
{
    size_t count = this->m_aModelObjects.size();
    for (size_t i = 0; i < count; i++)
    {
...
void OpenGL_011_Texturing::recreateSwapChain_Custom()
{   
    size_t count = this->m_aModelObjects.size();
    for (size_t i = 0; i < count; i++)
    {
