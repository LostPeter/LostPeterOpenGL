...
#include "opengl_009_instancing.h"
...
{
    "Assets/Shader/Common/standard_mesh_opaque.vert.spv", "Assets/Shader/Common/standard_mesh_opaque.frag.spv", //standard_mesh_opaque
    "Assets/Shader/Common/standard_mesh_transparent.vert.spv", "Assets/Shader/Common/standard_mesh_transparent.frag.spv", //standard_mesh_transparent
    "Assets/Shader/Common/standard_mesh_outline.vert.spv", "Assets/Shader/Common/standard_mesh_outline.frag.spv", //standard_mesh_outline
};
...
{
    "Assets/Shader/Common/standard_mesh_transparent", "Assets/Shader/Common/standard_mesh_outline", //viking_room
    "Assets/Shader/Common/standard_mesh_opaque", "Assets/Shader/Common/standard_mesh_outline", //bunny 
};
...
OpenGL_009_Instancing::OpenGL_009_Instancing(int width, int height, String name)
...
void OpenGL_009_Instancing::createCamera()
{
...
void OpenGL_009_Instancing::loadModel_Custom()
{
    for (int i = 0; i < g_CountLen; i++)
    {
        ...
        {
            String msg = "*********************** OpenGL_009_Instancing::loadModel_Custom: Failed to load model: " + pModelObject->pathModel;
        ...
        {   
            String msg = "*********************** OpenGL_009_Instancing::loadModel_Custom: Failed to load texture: " + pModelObject->pathTexture;
            F_LogError(msg.c_str());
            throw std::runtime_error(msg.c_str());
        }
...
bool OpenGL_009_Instancing::loadModel_VertexIndex(ModelObject* pModelObject, bool isFlipY, bool isTransformLocal, const FMatrix4& matTransformLocal)
{
    //1> Load 
    FMeshData meshData;
    ...
    {
        F_LogError("*********************** OpenGL_009_Instancing::loadModel_VertexIndex load model failed: [%s] !", pModelObject->pathModel.c_str());
        return false; 
    }
    ...
    F_LogInfo("OpenGL_009_Instancing::loadModel_VertexIndex: load model [%s] success, Vertex count: [%d], Index count: [%d] !", 
		...
		pModelObject->poIndexBuffer_Data != nullptr)
	{
		...
		{
			F_LogError("*********************** OpenGL_009_Instancing::loadModel_VertexIndex: create buffer vertex index failed: [%s] !", pModelObject->nameModel.c_str());
			return false;
		}
	}
	else
	{
		...
		{
			F_LogError("*********************** OpenGL_009_Instancing::loadModel_VertexIndex: create buffer vertex failed: [%s] !", pModelObject->nameModel.c_str());
			return false;
		}
	}
...
bool OpenGL_009_Instancing::loadModel_Texture(ModelObject* pModelObject)
{
    if (!pModelObject->pathTexture.empty())
    {
		...
		pModelObject->poTexture = new GLTexture(nameTexture,
												aPathTexture,
												F_Texture_2D,
        ...
        F_LogInfo("OpenGL_009_Instancing::loadModel_Texture: Load texture [%s] success !", pModelObject->pathTexture.c_str());
    }

    return true;
}

void OpenGL_009_Instancing::createCustomCB()
{
    rebuildInstanceCBs(true);
}
void OpenGL_009_Instancing::rebuildInstanceCBs(bool isCreateBuffer)
{
    ...
    for (size_t i = 0; i < count; i++)
    {
		...
		if (isCreateBuffer)
		{
			...
			{
				String msg = "*********************** OpenGL_009_Instancing::rebuildInstanceCBs: create buffer uniform: [" + nameBuffer + "] failed !";
		...
		if (isCreateBuffer)
		{
			F_DELETE(pModelObject->poBufferUniform_Outline)
...
void OpenGL_009_Instancing::createCustomBeforePipeline()
{
    //1> Shader
    createShaderModules();
}  
void OpenGL_009_Instancing::createGraphicsPipeline_Custom()
{
    String namePathBase;
    ...
    for (size_t i = 0; i < count; i++)
    {
        ...
        GLShader* pShaderVertex = findShaderModule(pathVertShaderBase);
        GLShader* pShaderFragment = findShaderModule(pathFragShaderBase);
        ...
        GLShader* pShaderVertex_Outline = findShaderModule(pathVertShaderOutline);
        GLShader* pShaderFragment_Outline = findShaderModule(pathFragShaderOutline);
        ...
        {
            String msg = "*********************** OpenGL_009_Instancing::createGraphicsPipeline_Custom: Failed to create pipeline stencil !";
            F_LogError(msg.c_str());
            throw std::runtime_error(msg.c_str());
        }
        ...
        {
            String msg = "*********************** OpenGL_009_Instancing::createGraphicsPipeline_Custom: Failed to create pipeline outline !";
            F_LogError(msg.c_str());
            throw std::runtime_error(msg.c_str());
        }
...
void OpenGL_009_Instancing::createShaderModules()
{
    ...
    for (int i = 0; i < g_ShaderCount; i++)
    {
        ...
        GLShader* pShaderVertex =  createShader(nameVertexShader, pathVert, F_Shader_Vertex);
        if (pShaderVertex == nullptr)
        {
            String msg = "*********************** OpenGL_009_Instancing::createShaderModules: Failed to create shader vertex: " + pathVert;
            F_LogError(msg.c_str());
            throw std::runtime_error(msg);
        }
        ...
        GLShader* pShaderFragment = createShader(nameFragmentShader, pathFrag, F_Shader_Fragment);
        if (pShaderFragment == nullptr)
        {
            String msg = "*********************** OpenGL_009_Instancing::createShaderModules: Failed to create shader fragment: " + pathFrag;
            F_LogError(msg.c_str());
            throw std::runtime_error(msg);
        }
...
void OpenGL_009_Instancing::createDescriptorSets_Custom()
{
    size_t count = this->m_aModelObjects.size();
    for (size_t i = 0; i < count; i++)
    {
...
void OpenGL_009_Instancing::updateCBs_Custom()
{
...
bool OpenGL_009_Instancing::beginRenderImgui()
{
    ImGui_ImplOpenGL3_NewFrame();
    ImGui_ImplGlfw_NewFrame();
    ImGui::NewFrame();
    static bool windowOpened = true;
    ImGui::Begin("OpenGL_009_Instancing", &windowOpened, 0);
    {
        //0> Common
        commonConfig();
...
void OpenGL_009_Instancing::modelConfig()
{
    if (ImGui::CollapsingHeader("Model Settings"))
    {
        ...
        for (size_t i = 0; i < count; i++)
        {
            ...
            String nameModel = FUtilString::SaveInt((int32)i) + " - " + pModelObject->nameModel;
...
void OpenGL_009_Instancing::endRenderImgui()
{
    OpenGLWindow::endRenderImgui();

}

void OpenGL_009_Instancing::drawMeshDefault_Custom()
{   
    ...
    for (size_t i = 0; i < count; i++)
    {
		...
		pModelObject->poStatePipelineGraphics_Stencil->poTypePolygonMode = pModelObject->poTypePolygonMode;
