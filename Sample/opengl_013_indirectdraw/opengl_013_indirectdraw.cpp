...
#include "opengl_013_indirectdraw.h"
...
void OpenGL_013_IndirectDraw::createCustomCB()
{
...
void OpenGL_013_IndirectDraw::createCustomBeforePipeline()
{
...
void OpenGL_013_IndirectDraw::createMeshes()
{
    for (int i = 0; i < g_MeshCount; i++)
    {
        ...
        {
            String msg = "*********************** OpenGL_013_IndirectDraw::createMeshes: create mesh: [" + nameMesh + "] failed !";
            F_LogError(msg.c_str());
            throw std::runtime_error(msg);
        }
        ...
        F_LogInfo("OpenGL_013_IndirectDraw::createMeshes: create mesh: [%s], vertex type: [%s], mesh type: [%s], geometry type: [%s], path: [%s] success !", 
...
void OpenGL_013_IndirectDraw::destroyTextures()
{
...
void OpenGL_013_IndirectDraw::createTextures()
{
    for (int i = 0; i < g_TextureCount; i++)
    {
        ...
        F_LogInfo("OpenGL_013_IndirectDraw::createTextures: create texture: [%s], type: [%s], isRT: [%s], path: [%s] success !", 
                  nameTexture.c_str(), 
                  nameType.c_str(), 
...
GLTexture* OpenGL_013_IndirectDraw::findTexture(const String& nameTexture)
{
...
void OpenGL_013_IndirectDraw::destroyShaderModules()
{
...
void OpenGL_013_IndirectDraw::createShaderModules()
{
    for (int i = 0; i < g_ShaderCount; i++)
    {
        ...
        if (pShader == nullptr)
        {
            String msg = "*********************** OpenGL_013_IndirectDraw::createShaderModules: Failed to create shader: shaderPath: " + shaderPath;
            F_LogError(msg.c_str());
            throw std::runtime_error(msg);
        }
        this->m_aGLShaderModules.push_back(pShader);
        this->m_mapGLShaderModules[shaderName] = pShader;
        F_LogInfo("OpenGL_013_IndirectDraw::createShaderModules: create shader [%s] success !", shaderPath.c_str());
    }
}
GLShader* OpenGL_013_IndirectDraw::findShaderModule(const String& pathShaderModule)
{
...
void OpenGL_013_IndirectDraw::destroyDescriptorSetLayouts()
{
...
void OpenGL_013_IndirectDraw::createDescriptorSetLayouts()
{
...
DescriptorSetLayout* OpenGL_013_IndirectDraw::findDescriptorSetLayout(const String& nameDescriptorSetLayout)
{
...
bool OpenGL_013_IndirectDraw::beginRenderImgui()
{
    ImGui_ImplOpenGL3_NewFrame();
    ImGui_ImplGlfw_NewFrame();
    ImGui::NewFrame();
    static bool windowOpened = true;
    ImGui::Begin("OpenGL_013_IndirectDraw", &windowOpened, 0);
    {
        //0> Common
        commonConfig();
...
void OpenGL_013_IndirectDraw::modelConfig()
{
...
void OpenGL_013_IndirectDraw::endRenderImgui()
{
...
void OpenGL_013_IndirectDraw::recreateSwapChain_Custom()
{   
    size_t count = this->m_aModelObjects.size();
    for (size_t i = 0; i < count; i++)
    {
