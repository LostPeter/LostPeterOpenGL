...
bool OpenGL_012_Shadering::ModelMesh::LoadMesh(bool isFlipY, bool isTransformLocal, const FMatrix4& matTransformLocal)
{
	...
	else if (this->typeMesh == F_Mesh_Geometry)
	{
        ...
        {
            F_LogError("*********************** OpenGL_012_Shadering::ModelMesh::LoadMesh: create geometry mesh failed: typeGeometry: [%s] !", F_GetMeshGeometryTypeName(this->typeGeometryType).c_str());
            return false; 
        }
	}
	else
    {
        F_Assert(false && "OpenGL_012_Shadering::ModelMesh::LoadMesh: Wrong typeMesh !")
        return false;
    }
		...
		this->poIndexBuffer_Data != nullptr)
	{
		...
		{
			F_LogError("*********************** OpenGL_012_Shadering::loadModel_VertexIndex: create buffer vertex index failed: [%s] !", this->nameMesh.c_str());
			return false;
		}
	}
	else
	{
		...
		{
			F_LogError("*********************** OpenGL_012_Shadering::loadModel_VertexIndex: create buffer vertex failed: [%s] !", this->nameMesh.c_str());
			return false;
		}
	}
...
void OpenGL_012_Shadering::createGraphicsPipeline_Custom()
{
    ...
    for (size_t i = 0; i < count; i++)
    {
        ...
        {
            String msg = "*********************** OpenGL_012_Shadering::createGraphicsPipeline_Custom: Failed to create pipeline graphics !";
...
void OpenGL_012_Shadering::createComputePipeline_Custom()
{
    ...
    for (size_t i = 0; i < count; i++)
    {
        ...
        F_Assert(aShaderComps.size() == aDSLs.size() && "OpenGL_012_Shadering::createComputePipeline_Custom")
        ...
        for (size_t j = 0; j < count_cs; j++)
        {
            ...
            {
                String msg = "*********************** OpenGL_012_Shadering::createComputePipeline_Custom: Failed to create pipeline compute !";
                F_LogError(msg.c_str());
                throw std::runtime_error(msg.c_str());
            }
        ...
        if (pModelObject->isUsedCompute)
		{
            ...
            for (int j = 0; j < count_comp; j++)
            {
                ...
                for (size_t p = 0; p < count_names; p++)
                {
                    ...
                    {
                        String msg = "*********************** OpenGL_012_Shadering::createDescriptorSets_Custom: Compute: Wrong DescriptorSetLayout type: " + nameDescriptorSet;
                        F_LogError(msg.c_str());
                        throw std::runtime_error(msg.c_str());
                    }
                }
...
void OpenGL_012_Shadering::updateCompute_BeforeRender_Custom()
{
	size_t count = this->m_aModelObjects.size();
    for (size_t i = 0; i < count; i++)
	{
