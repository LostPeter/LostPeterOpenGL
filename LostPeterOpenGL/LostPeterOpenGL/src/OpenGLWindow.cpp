    ...
    void OpenGLWindow::loadAssets()
    {
        ...
        void OpenGLWindow::loadGeometry()
        {
            ...
            void OpenGLWindow::loadVertexIndexBuffer()
            {
                ...
                void OpenGLWindow::updateBufferIndirectCommand_DrawInstance(GLBufferIndirectCommand* pBufferIndirectCommand)
				{
                ...
                void OpenGLWindow::updateBufferIndirectCommand_DrawIndexedInstance(GLBufferIndirectCommand* pBufferIndirectCommand)
				{
					pBufferIndirectCommand->UpdateBuffer();
				}
                ...
                void OpenGLWindow::bindGLBufferUniformBlockIndex(uint32 nBufferUniformID, uint32 bindingIndex, size_t offset, size_t bufSize)
                {
                    glBindBuffer(GL_UNIFORM_BUFFER, nBufferUniformID);
															   ...
															   uint32& nBufferIndirectCommandID)
				{
                    ...
                    if (GL_NO_ERROR != error) 
                    {
                        F_LogError("*********************** OpenGLWindow::createGLBufferIndirectCommand: create indirect command buffer error, glGenBuffers GL error: [%u] !", error);
                    }
                    ...
                    if (GL_NO_ERROR != error) 
                    {
                        F_LogError("*********************** OpenGLWindow::createGLBufferIndirectCommand: create indirect command buffer error, glBufferData GL error: [%u] !", error);
                    }
                    ...
                    if (GL_NO_ERROR != error) 
                    {
                        F_LogError("*********************** OpenGLWindow::createGLBufferIndirectCommand: create indirect command buffer error, glBindBufferRange GL error: [%u] !", error);
                    }
            ...
            void OpenGLWindow::loadTexture()
            {
				...
				bool OpenGLWindow::createTexture2DArray(const String& nameTexture,
													    ...
													    uint32& nTextureID)
				{
					...
					if (count_tex <= 0)
					{
						F_LogError("*********************** OpenGLWindow::createTexture2DArray: Texture path count <= 0 !");
						return false;
					}
					for (size_t i = 0; i < count_tex; i++)
					{
						...
						if (!pixels) 
						{
							...
							String msg = "*********************** OpenGLWindow::createTexture2DArray: Failed to load texture image: " + pathTexture;
							F_LogError(msg.c_str());
							throw std::runtime_error(msg);
						}
					...
					for (size_t i = 1; i < count_tex; i++)
					{
						...
						{
							s_DeletePixels(aPixels);
							String msg = "*********************** OpenGLWindow::createTexture2DArray: Texture image's all width must the same !";
							F_LogError(msg.c_str());
							throw std::runtime_error(msg);
						}
						if (aHeight[i] != height)
						{
							s_DeletePixels(aPixels);
							String msg = "*********************** OpenGLWindow::createTexture2DArray: Texture image's all height must the same !";
							F_LogError(msg.c_str());
							throw std::runtime_error(msg);
						}
					}
                    ...
                    if (!createGLTexture(nameTexture,
                                         ...
                                         nTextureID))
                    {
                        F_LogError("*********************** OpenGLWindow::createTexture2DArray: Failed to create texture, name: [%s] !", nameTexture.c_str());
						F_DELETE_T(pData)
                        return false;
                    }
                    F_DELETE_T(pData)

                    F_LogInfo("OpenGLWindow::createTexture2DArray: Success to create texture, name: [%s] !", nameTexture.c_str());
                    return true;
				}

				bool OpenGLWindow::createTexture3D(const String& nameTexture,
                                                   const String& pathAsset_Tex,
                                                   int& mipMapCount, 
                                                   bool isAutoMipmap,
                                                   FTextureType typeTexture, 
                                                   ...
                                                   uint32& nTextureID)
				{
                    ...
                    if (!pixels) 
                    {
                        String msg = "*********************** OpenGLWindow::createTexture3D: Failed to load texture image: " + pathAsset_Tex;
                        F_LogError(msg.c_str());
                        throw std::runtime_error(msg);
                    }
                    ...
                    if (!createGLTexture(nameTexture,
                                         ...
                                         nTextureID))
                    {
                        F_LogError("*********************** OpenGLWindow::createTexture3D: Failed to create texture, name: [%s], path: [%s] !", nameTexture.c_str(), pathAsset_Tex.c_str());
                        stbi_image_free(pixels);
                        return false;
                    }
                    stbi_image_free(pixels);

                    F_LogInfo("OpenGLWindow::createTexture3D: Success to create texture, name: [%s], path: [%s] !", nameTexture.c_str(), pathAsset_Tex.c_str());
                    return true;
				}
                bool OpenGLWindow::createTexture3D(const String& nameTexture,
                                                   uint8* pDataRGBA,
                                                   ...
                                                   uint32& nTextureID)
				{
                    ...
                    if (!createGLTexture(nameTexture,
                                         ...
                                         nTextureID))
                    {
                        F_LogError("*********************** OpenGLWindow::createTexture3D: Failed to create texture, name: [%s] !", nameTexture.c_str());
                        return false;
                    }

                    F_LogInfo("OpenGLWindow::createTexture3D: Success to create texture, name: [%s] !", nameTexture.c_str());
                    return true;
				}

				bool OpenGLWindow::createTextureCubeMap(const String& nameTexture,
													    const StringVector& aPathAsset_Tex, 
													    int& mipMapCount, 
													    bool isAutoMipmap,
													    FTextureType typeTexture, 
													    ...
													    uint32& nTextureID)
				{
					...
					if (count_tex <= 0)
					{
						F_LogError("*********************** OpenGLWindow::createTextureCubeMap: Texture path count <= 0 !");
						return false;
					}
					for (size_t i = 0; i < count_tex; i++)
					{
						...
						{
							s_DeletePixels(aPixels);
							String msg = "*********************** OpenGLWindow::createTextureCubeMap: Failed to load texture image: " + pathTexture;
							F_LogError(msg.c_str());
							throw std::runtime_error(msg);
						}
					...
					for (size_t i = 1; i < count_tex; i++)
					{
						...
						{
							s_DeletePixels(aPixels);
							String msg = "*********************** OpenGLWindow::createTextureCubeMap: Texture image's all width must the same !";
							F_LogError(msg.c_str());
							throw std::runtime_error(msg);
						}
						if (aHeight[i] != height)
						{
							s_DeletePixels(aPixels);
							String msg = "*********************** OpenGLWindow::createTextureCubeMap: Texture image's all height must the same !";
							F_LogError(msg.c_str());
							throw std::runtime_error(msg);
						}
					}
                    ...
                    if (!createGLTexture(nameTexture,
                                         ...
                                         nTextureID))
                    {
                        F_LogError("*********************** OpenGLWindow::createTextureCubeMap: Failed to create texture, name: [%s] !", nameTexture.c_str());
						F_DELETE_T(pData)
                        return false;
                    }
                    F_DELETE_T(pData)

                    F_LogInfo("OpenGLWindow::createTextureCubeMap: Success to create texture, name: [%s] !", nameTexture.c_str());
                    return true;
				}
				...
				bool OpenGLWindow::createTextureRenderTarget2DArray(const String& nameTexture,
                                                                    const FVector4& clDefault,
                                                                    bool isSetColor,
                                                                    int channel,
                                                                    int width, 
                                                                    ...
                                                                    uint32& nTextureID)
				{
                    ...
                    {
                        F_DELETE_T(pData)
                        F_LogError("*********************** OpenGLWindow::createTextureRenderTarget2DArray: Failed to create texture RenderTarget2DArray, name: [%s] !", nameTexture.c_str());
                        return false;
                    }
					F_DELETE_T(pData)
                                                                    ...
                                                                    uint32& nTextureID)
				{
                    ...
                    F_LogInfo("OpenGLWindow::createTextureRenderTarget2DArray: Success to create texture RenderTarget2DArray, name: [%s] !", nameTexture.c_str());
                    return true;
				}

				bool OpenGLWindow::createTextureRenderTarget3D(const String& nameTexture,
															   const FVector4& clDefault,
															   bool isSetColor,
															   int channel,
															   int width, 
															   ...
															   uint32& nTextureID)
				{
                    ...
                    {
                        F_DELETE_T(pData)
                        F_LogError("*********************** OpenGLWindow::createTextureRenderTarget3D: Failed to create texture RenderTarget3D, name: [%s] !", nameTexture.c_str());
                        return false;
                    }
					F_DELETE_T(pData)
															   ...
															   uint32& nTextureID)
				{
                    ...
                    F_LogInfo("OpenGLWindow::createTextureRenderTarget3D: Success to create texture RenderTarget3D, name: [%s] !", nameTexture.c_str());
                    return true;
				}

				bool OpenGLWindow::createTextureRenderTargetCubeMap(const String& nameTexture,
																	const FVector4& clDefault,
																	bool isSetColor,
																	int channel,
																	int width, 
																	...
																	uint32& nTextureID)
				{
                    ...
                    {
                        F_DELETE_T(pData)
                        F_LogError("*********************** OpenGLWindow::createTextureRenderTargetCubeMap: Failed to create texture RenderTargetCubeMap, name: [%s] !", nameTexture.c_str());
                        return false;
                    }
					F_DELETE_T(pData)
																	...
																	uint32& nTextureID)
				{
                    ...
                    F_LogInfo("OpenGLWindow::createTextureRenderTargetCubeMap: Success to create texture RenderTargetCubeMap, name: [%s] !", nameTexture.c_str());
                    return true;
				}
                ...
                void OpenGLWindow::createComputePipeline_Custom()
                {
                                                                                     ...
                                                                                     GLShader* pShaderCompute)
					{
                        ...
                        if (!pStatePipelineCompute->Init(pDSL,
                                                         pShaderCompute)) 
                        {
                            F_LogError("*********************** OpenGLWindow::createStatePipelineCompute failed, name: [%s] !", nameStatePipelineCompute.c_str());
        ...
        void OpenGLWindow::updateRender()
        {
            ...
            void OpenGLWindow::updateRenderCommandBuffers_Default()
            {
                                                   ...
                                                   int stencil)
                {
					...
					void OpenGLWindow::drawInstanceIndirect(GLenum mode, const void* pIndirect)
					{
						glDrawArraysIndirect(mode, pIndirect);
					}
					void OpenGLWindow::drawIndexedInstanceIndirect(GLenum mode, GLenum type, const void* pIndirect)
					{
						glDrawElementsIndirect(mode, type, pIndirect);
					}
					void OpenGLWindow::drawMultiInstanceIndirect(GLenum mode, const void* pIndirect, GLsizei drawcount, GLsizei stride)
					{
						glMultiDrawArraysIndirect(mode, pIndirect, drawcount, stride);
					}
					void OpenGLWindow::drawMultiIndexedInstanceIndirect(GLenum mode, GLenum type, const void* pIndirect, GLsizei drawcount, GLsizei stride)
					{
						glMultiDrawElementsIndirect(mode, type, pIndirect, drawcount, stride);
					}
