...
#include "opengl_011_texturing.h"
...
OpenGL_011_Texturing::OpenGL_011_Texturing(int width, int height, String name)
...
void OpenGL_011_Texturing::createCamera()
{
...
void OpenGL_011_Texturing::loadModel_Custom()
{
    for (int i = 0; i < g_CountLen; i++)
    {
        ...
        {
            String msg = "*********************** OpenGL_011_Texturing::loadModel_Custom: Failed to load model: " + pModelObject->pathModel;
        ...
        {   
            String msg = "*********************** OpenGL_011_Texturing::loadModel_Custom: Failed to load texture: " + pModelObject->pathTexture;
            F_LogError(msg.c_str());
            throw std::runtime_error(msg.c_str());
        }
...
bool OpenGL_011_Texturing::loadModel_VertexIndex(ModelObject* pModelObject, bool isFlipY, bool isTransformLocal, const FMatrix4& matTransformLocal)
{
    //1> Load 
    FMeshData meshData;
    ...
    {
        F_LogError("*********************** OpenGL_011_Texturing::loadModel_VertexIndex load model failed: [%s] !", pModelObject->pathModel.c_str());
        return false; 
    }
    ...
    F_LogInfo("OpenGL_011_Texturing::loadModel_VertexIndex: load model [%s] success, Vertex count: [%d], Index count: [%d] !", 
		...
		pModelObject->poIndexBuffer_Data != nullptr)
	{
		...
		{
			F_LogError("*********************** OpenGL_011_Texturing::loadModel_VertexIndex: create buffer vertex index failed: [%s] !", pModelObject->nameModel.c_str());
			return false;
		}
	}
	else
	{
		...
		{
			F_LogError("*********************** OpenGL_011_Texturing::loadModel_VertexIndex: create buffer vertex failed: [%s] !", pModelObject->nameModel.c_str());
			return false;
		}
	}
...
bool OpenGL_011_Texturing::loadModel_Texture(ModelObject* pModelObject)
{
    if (!pModelObject->pathTexture.empty())
    {
		...
		{
			F_LogError("*********************** OpenGL_011_Texturing::loadModel_Texture: Failed to create texture, path: [%s] !", pModelObject->pathTexture.c_str());
			F_DELETE(this->poTexture)
			return false;
		}
        F_LogInfo("OpenGL_011_Texturing::loadModel_Texture: Load texture [%s] success !", pModelObject->pathTexture.c_str());
    }

    return true;
}

void OpenGL_011_Texturing::createCustomCB()
{
    rebuildInstanceCBs(true);
}
void OpenGL_011_Texturing::rebuildInstanceCBs(bool isCreateBuffer)
{
    ...
    for (size_t i = 0; i < count; i++)
    {
		...
		if (isCreateBuffer)
		{
			...
			{
				String msg = "*********************** OpenGL_011_Texturing::rebuildInstanceCBs: create buffer uniform: [" + nameBuffer + "] failed !";
...
void OpenGL_011_Texturing::createCustomBeforePipeline()
{
    //1> Shader
    createShaderModules();
}  
void OpenGL_011_Texturing::createGraphicsPipeline_Custom()
{
    String namePathBase;
    ...
    for (size_t i = 0; i < count; i++)
    {
        ...
        {
            String msg = "*********************** OpenGL_011_Texturing::createGraphicsPipeline_Custom: Failed to create pipeline stencil !";
            F_LogError(msg.c_str());
            throw std::runtime_error(msg.c_str());
        }
    }
...
void OpenGL_011_Texturing::createShaderModules()
{
    ...
    for (int i = 0; i < g_ShaderCount; i++)
    {
        ...
        {
            String msg = "*********************** OpenGL_011_Texturing::createShaderModules: Failed to create shader vertex: " + pathVert;
            F_LogError(msg.c_str());
            throw std::runtime_error(msg);
        }
        this->m_aGLShaderModules.push_back(pShaderVertex);
        this->m_mapGLShaderModules[pathVert] = pShaderVertex;
        F_LogInfo("OpenGL_011_Texturing::createShaderModules: create shader [%s] success !", pathVert.c_str());
        ...
        {
            String msg = "*********************** OpenGL_011_Texturing::createShaderModules: Failed to create shader fragment: " + pathFrag;
            F_LogError(msg.c_str());
            throw std::runtime_error(msg);
        }
        this->m_aGLShaderModules.push_back(pShaderFragment);
        this->m_mapGLShaderModules[pathFrag] = pShaderFragment;
        F_LogInfo("OpenGL_011_Texturing::createShaderModules: create shader [%s] success !", pathFrag.c_str());
    }
}
GLShader* OpenGL_011_Texturing::findShaderModule(const String& pathShaderModule)
{
...
void OpenGL_011_Texturing::createDescriptorSets_Custom()
{
    size_t count = this->m_aModelObjects.size();
    for (size_t i = 0; i < count; i++)
    {
...
void OpenGL_011_Texturing::updateCBs_Custom()
{
...
bool OpenGL_011_Texturing::beginRenderImgui()
{
    ImGui_ImplOpenGL3_NewFrame();
    ImGui_ImplGlfw_NewFrame();
    ImGui::NewFrame();
    static bool windowOpened = true;
    ImGui::Begin("OpenGL_011_Texturing", &windowOpened, 0);
    {
        //0> Common
        commonConfig();
...
void OpenGL_011_Texturing::modelConfig()
{
...
void OpenGL_011_Texturing::endRenderImgui()
{
    OpenGLWindow::endRenderImgui();

}

void OpenGL_011_Texturing::drawMeshDefault_Custom()
{   
...
void OpenGL_011_Texturing::cleanupCustom()
{
