...
#include "opengl_012_shadering.h"
...
////High-Level Texture Operation
    ...
    GL_BACK, //textureDisplacementMap
...
/////////////////////////// ModelMesh ///////////////////////////
bool OpenGL_012_Shadering::ModelMesh::LoadMesh(bool isFlipY, bool isTransformLocal, const FMatrix4& matTransformLocal)
{
    //1> Load
    FMeshData meshData;
    ...
    {
        F_LogError("*********************** OpenGL_012_Shadering::ModelMesh::LoadMesh: load mesh failed: [%s] !", this->pathMesh.c_str());
        return false; 
    }
    ...
    if (this->poTypeVertex == F_MeshVertex_Pos3Color4Normal3Tex2)
    {
        ...
        F_LogInfo("OpenGL_012_Shadering::ModelMesh::LoadMesh: load mesh [%s] success, [Pos3Color4Normal3Tex2]: Vertex count: [%d], Index count: [%d] !", 
                  this->nameMesh.c_str(),
    ...
    else if (this->poTypeVertex == F_MeshVertex_Pos3Color4Normal3Tangent3Tex2)
    {
        ...
        F_LogInfo("OpenGL_012_Shadering::ModelMesh::LoadMesh: load mesh [%s] success, [Pos3Color4Normal3Tangent3Tex2]: Vertex count: [%d], Index count: [%d] !", 
                  this->nameMesh.c_str(),
...
OpenGL_012_Shadering::OpenGL_012_Shadering(int width, int height, String name)
...
void OpenGL_012_Shadering::createCamera()
{
    OpenGLWindow::createCamera();
    
    cameraReset();
}
void OpenGL_012_Shadering::cameraReset()
{
...
void OpenGL_012_Shadering::loadModel_Custom()
{
    createMeshes();
    createTextures();
    createShaderModules();
    ...
    for (int i = 0; i < g_ObjectCount; i++)
    {
        ...
        //Mesh
        {
            ...
            F_Assert(pMesh != nullptr && "OpenGL_012_Shadering::loadModel_Custom")
        ...
        //Texture TESC
        {
            ...
            if (!nameTextureTESC.empty())
            {
                ...
                for (size_t j = 0; j < count_tex; j++)
                {
                    ...
                    GLTexture* pTextureTESC = this->findTexture(nameTex);
        ...
        //Texture TESE
        {
            ...
            if (!nameTextureTESE.empty())
            {
                ...
                for (size_t j = 0; j < count_tex; j++)
                {
                    ...
                    GLTexture* pTextureTESE = this->findTexture(nameTex);
        ...
        //Texture GS
        {
            ...
            if (!nameTextureGS.empty())
            {
                ...
                for (size_t j = 0; j < count_tex; j++)
                {
                    ...
                    GLTexture* pTextureGS = this->findTexture(nameTex);
...
void OpenGL_012_Shadering::createCustomCB()
{
    rebuildInstanceCBs(true);
}
void OpenGL_012_Shadering::rebuildInstanceCBs(bool isCreateBuffer)
{
    ...
    for (size_t i = 0; i < count; i++)
    {
            ...
            //Texture FS
            {
                ...
                if (pTextureFSs != nullptr)
                {
                    ...
                    for (int p = 0; p < count_texture; p++)
                    {
                        ...
                        materialConstants.aTexLayers[p].texWidth = (float)pTexture->width;
                        ...
                        else if (pModelObject->nameObject == g_Object_TextureAnimation_Chunk) //TextureAnimation_Chunk
                        {
                            ...
                            {
                                materialConstants.aTexLayers[p].texChunkMaxX = (float)pTexture->texChunkMaxX;
                                materialConstants.aTexLayers[p].texChunkMaxY = (float)pTexture->texChunkMaxY;
                                int indexX = pTexture->texChunkIndex % pTexture->texChunkMaxX;
                                int indexZ = pTexture->texChunkIndex / pTexture->texChunkMaxX;
                                materialConstants.aTexLayers[p].texChunkIndexX = (float)indexX;
                                materialConstants.aTexLayers[p].texChunkIndexY = (float)indexZ;
                            }
                        }       
		...
		if (isCreateBuffer)
		{
			...
			{
				String msg = "*********************** OpenGL_012_Shadering::rebuildInstanceCBs: create buffer uniform: [" + nameBuffer + "] failed !";
...
void OpenGL_012_Shadering::createCustomBeforePipeline()
{
	
}  
void OpenGL_012_Shadering::createGraphicsPipeline_Custom()
{
    size_t count = this->m_aModelObjects.size();
    for (size_t i = 0; i < count; i++)
    {
        ...
        {
            String msg = "*********************** OpenGL_012_Shadering::createGraphicsPipeline_Custom: Failed to create pipeline stencil !";
...
void OpenGL_012_Shadering::createComputePipeline_Custom()
{

}

void OpenGL_012_Shadering::destroyMeshes()
{
...
void OpenGL_012_Shadering::createMeshes()
{
    for (int i = 0; i < g_MeshCount; i++)
    {
        ...
        {
            String msg = "*********************** OpenGL_012_Shadering::createMeshes: create mesh: [" + nameMesh + "] failed !";
            F_LogError(msg.c_str());
            throw std::runtime_error(msg);
        }
        ...
        F_LogInfo("OpenGL_012_Shadering::createMeshes: create mesh: [%s], vertex type: [%s], mesh type: [%s], geometry type: [%s], path: [%s] success !", 
                  nameMesh.c_str(), nameVertexType.c_str(), nameMeshType.c_str(), nameGeometryType.c_str(), pathMesh.c_str());
    }
}
OpenGL_012_Shadering::ModelMesh* OpenGL_012_Shadering::findMesh(const String& nameMesh)
{
...
void OpenGL_012_Shadering::destroyTextures()
{
...
void OpenGL_012_Shadering::createTextures()
{
    for (int i = 0; i < g_TextureCount; i++)
    {
        ...
        F_LogInfo("OpenGL_012_Shadering::createTextures: create texture: [%s], type: [%s], isRT: [%s], path: [%s] success !", 
                  nameTexture.c_str(), 
                  nameType.c_str(), 
...
GLTexture* OpenGL_012_Shadering::findTexture(const String& nameTexture)
{
...
void OpenGL_012_Shadering::destroyShaderModules()
{
...
void OpenGL_012_Shadering::createShaderModules()
{
    for (int i = 0; i < g_ShaderCount; i++)
    {
        ...
        if (pShader == nullptr)
        {
            String msg = "*********************** OpenGL_012_Shadering::createShaderModules: Failed to create shader: shaderPath: " + shaderPath;
            F_LogError(msg.c_str());
            throw std::runtime_error(msg);
        }
        this->m_aGLShaderModules.push_back(pShader);
        this->m_mapGLShaderModules[shaderName] = pShader;
        F_LogInfo("OpenGL_012_Shadering::createShaderModules: create shader [%s] success !", shaderPath.c_str());
    }
}
GLShader* OpenGL_012_Shadering::findShaderModule(const String& pathShaderModule)
{
...
void OpenGL_012_Shadering::destroyDescriptorSetLayouts()
{
...
void OpenGL_012_Shadering::createDescriptorSetLayouts()
{
...
DescriptorSetLayout* OpenGL_012_Shadering::findDescriptorSetLayout(const String& nameDescriptorSetLayout)
{
...
void OpenGL_012_Shadering::createDescriptorSets_Custom()
{
    size_t count = this->m_aModelObjects.size();
    for (size_t i = 0; i < count; i++)
    {
		...
		//Pipeline Graphics
		{
            ...
            for (int j = 0; j < count_ds; j++)
            {
                ...
                {
                    String msg = "*********************** OpenGL_012_Shadering::createDescriptorSets_Custom: Graphics: Wrong DescriptorSetLayout type: " + nameDescriptorSet;
                    F_LogError(msg.c_str());
                    throw std::runtime_error(msg.c_str());
                }
            }
...
void OpenGL_012_Shadering::updateCBs_Custom()
{
                    ...
                    for (size_t p = 0; p < count_texture; p++)
                    {
                            ...
                            pTexture->texChunkMaxY > 0)
                        {
                            if (++ pTexture->frameCurrent >= 30)
                            {
                                ...
                                int indexX = pTexture->texChunkIndex % pTexture->texChunkMaxX;
                                int indexZ = pTexture->texChunkIndex / pTexture->texChunkMaxX;
                                materialCB.aTexLayers[0].texChunkIndexX = (float)indexX;
                                materialCB.aTexLayers[0].texChunkIndexY = (float)indexZ;
                            }   
                        }
                    }
                }
...
bool OpenGL_012_Shadering::beginRenderImgui()
{
    ImGui_ImplOpenGL3_NewFrame();
    ImGui_ImplGlfw_NewFrame();
    ImGui::NewFrame();
    static bool windowOpened = true;
    ImGui::Begin("OpenGL_012_Shadering", &windowOpened, 0);
    {
        //0> Common
        commonConfig();
...
void OpenGL_012_Shadering::modelConfig()
{
                                            ...
                                            if (ImGui::CollapsingHeader(nameMaterial_Texture.c_str()))
                                            {
                                                ...
                                                else 
                                                {
                                                    if (pModelObject->nameObject == g_Object_TextureBumpMap) //TextureBumpMap
                                                    {
                                                        ...
                                                        {
                                                            if (s_aBumpMappingDescs[nIndex].Value == (int)mat.aTexLayers[p].indexTextureArray)
                                                                break;
                                                        }
                                                        ...
                                                        if (ImGui::BeginCombo(nameBumpMappingType.c_str(), preview_text))
                                                        {
                                                            for (int q = 0; q < IM_ARRAYSIZE(s_aBumpMappingDescs); q++)
                                                            {
                                                                ...
                                                                {
                                                                    mat.aTexLayers[p].indexTextureArray = (float)s_aBumpMappingDescs[q].Value;
                                                                    break;
                                                                }
                                                            }
                                                            ImGui::EndCombo();
                                                    ...
                                                    else if (pModelObject->nameObject == g_Object_TextureNormalMap) //TextureNormalMap
                                                    {
                                                        ...
                                                        if (ImGui::BeginCombo(nameNormalMappingType.c_str(), preview_text))
                                                        {
                                                            for (int q = 0; q < IM_ARRAYSIZE(s_aNormalMappingDescs); q++)
                                                            {
                                                                ...
                                                                {
                                                                    mat.aTexLayers[p].indexTextureArray = (float)s_aNormalMappingDescs[q].Value;
                                                                    break;
                                                                }
                                                            }
                                                            ImGui::EndCombo();
                                                    ...
                                                    else if (pModelObject->nameObject == g_Object_TextureParallaxMap) //TextureParallaxMap
                                                    {
                                                        ...
                                                        if (ImGui::BeginCombo(nameParallaxMappingType.c_str(), preview_text))
                                                        {
                                                            for (int q = 0; q < IM_ARRAYSIZE(s_aParallaxMappingDescs); q++)
                                                            {
                                                                ...
                                                                {
                                                                    mat.aTexLayers[p].indexTextureArray = (float)s_aParallaxMappingDescs[q].Value;
                                                                    break;
                                                                }
                                                            }
                                                            ImGui::EndCombo();
                                                        ...
                                                        {
                                                            mat.aTexLayers[p].texSpeedW = (float)numLayers;
                                                        }
                                                    }
                                                    else
                                                    {
...
void OpenGL_012_Shadering::endRenderImgui()
{
    OpenGLWindow::endRenderImgui();

}

void OpenGL_012_Shadering::drawMeshDefault_Custom()
{   
    ...
    for (size_t i = 0; i < count; i++)
    {
		...
		{	
			F_Assert(false && "OpenGL_012_Shadering::drawMeshDefault_Custom")
		}
        pModelObject->poStatePipelineGraphics->UnBindState();
    }
}

void OpenGL_012_Shadering::cleanupCustom()
{
...
void OpenGL_012_Shadering::cleanupSwapChain_Custom()
{
    size_t count = this->m_aModelObjects.size();
    for (size_t i = 0; i < count; i++)
    {
...
void OpenGL_012_Shadering::recreateSwapChain_Custom()
{   
    size_t count = this->m_aModelObjects.size();
    for (size_t i = 0; i < count; i++)
    {
