...
OpenGL_006_Depth::OpenGL_006_Depth(int width, int height, String name)
...
void OpenGL_006_Depth::createCamera()
{
...
void OpenGL_006_Depth::loadModel_Custom()
{
    for (int i = 0; i < g_CountLen; i++)
    {
        ...
        {
            String msg = "*********************** OpenGL_006_Depth::loadModel_Custom: Failed to load model: " + pModelObject->pathModel;
        ...
        {   
            String msg = "*********************** OpenGL_006_Depth::loadModel_Custom: Failed to load texture: " + pModelObject->pathTexture;
            F_LogError(msg.c_str());
            throw std::runtime_error(msg.c_str());
        }
...
bool OpenGL_006_Depth::loadModel_VertexIndex(ModelObject* pModelObject, bool isFlipY, bool isTransformLocal, const FMatrix4& matTransformLocal)
{
    //1> Load 
    FMeshData meshData;
    ...
    {
        F_LogError("*********************** OpenGL_006_Depth::loadModel_VertexIndex load model failed: [%s] !", pModelObject->pathModel.c_str());
        return false; 
    }
    ...
    F_LogInfo("OpenGL_006_Depth::loadModel_VertexIndex: load model [%s] success, Vertex count: [%d], Index count: [%d] !", 
		...
		pModelObject->poIndexBuffer_Data != nullptr)
	{
		...
		{
			F_LogError("*********************** OpenGL_006_Depth::loadModel_VertexIndex: create buffer vertex index failed: [%s] !", pModelObject->nameModel.c_str());
			return false;
		}
	}
	else
	{
		...
		{
			F_LogError("*********************** OpenGL_006_Depth::loadModel_VertexIndex: create buffer vertex failed: [%s] !", pModelObject->nameModel.c_str());
			return false;
		}
	}
...
bool OpenGL_006_Depth::loadModel_Texture(ModelObject* pModelObject)
{
    if (!pModelObject->pathTexture.empty())
    {
        ...
        F_LogInfo("OpenGL_006_Depth::loadModel_Texture: Load texture [%s] success !", pModelObject->pathTexture.c_str());
    }

    return true;
}

void OpenGL_006_Depth::createCustomCB()
{
    ...
    for (size_t i = 0; i < count; i++)
    {
		...
		{
			String msg = "*********************** OpenGL_006_Depth::createCustomCB: create buffer uniform: [" + nameBuffer + "] failed !";
...
void OpenGL_006_Depth::createGraphicsPipeline_Custom()
{
    String namePathBase;
	...
	{
		String msg = "*********************** OpenGL_006_Depth::createGraphicsPipeline_Custom: Failed to create shader vertex: " + this->shaderVertex_Path;
		F_LogError(msg.c_str());
		throw std::runtime_error(msg);
	}
	...
	{
		String msg = "*********************** OpenGL_006_Depth::createGraphicsPipeline_Custom: Failed to create shader fragment: " + this->shaderFragment_Path;
		F_LogError(msg.c_str());
		throw std::runtime_error(msg);
	}
    ...
    for (size_t i = 0; i < count; i++)
    {
        ...
        {
            String msg = "*********************** OpenGL_006_Depth::createGraphicsPipeline_Custom: Failed to create pipeline !";
            F_LogError(msg.c_str());
            throw std::runtime_error(msg.c_str());
        }
    }
...
void OpenGL_006_Depth::createDescriptorSets_Custom()
{
    size_t count = this->m_aModelObjects.size();
    for (size_t i = 0; i < count; i++)
    {
...
void OpenGL_006_Depth::updateCBs_Custom()
{
...
bool OpenGL_006_Depth::beginRenderImgui()
{
	ImGui_ImplOpenGL3_NewFrame();
    ImGui_ImplGlfw_NewFrame();
    ImGui::NewFrame();
    static bool windowOpened = true;
    ImGui::Begin("OpenGL_006_Depth", &windowOpened, 0);
    {
        //0> Common
        commonConfig();
...
void OpenGL_006_Depth::modelConfig()
{
...
void OpenGL_006_Depth::endRenderImgui()
{
    OpenGLWindow::endRenderImgui();

}

void OpenGL_006_Depth::drawMeshDefault_Custom()
{   
    size_t count = this->m_aModelObjects.size();
    for (size_t i = 0; i < count; i++)
    {
