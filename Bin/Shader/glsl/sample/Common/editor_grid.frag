...
vec4 Grid(VSOutput_Pos4Color4TexCood2 input, float divisions, vec4 color)
{
    float lineWidth = 2.0;

    vec2 vCoord = input.outTexCoord.xy * divisions;
...
void main()
{
	...
	vec4 vGrid1 = Grid(input, fDivs, color) + Grid(input, fDivs / subdivisions, color);

	fDivs = divisions / pow(2, round((abs(viewPos.y + 50) - step / subdivisions) / step));
	vec4 vGrid2 = Grid(input, fDivs, color) + Grid(input, fDivs / subdivisions, color);

	float fAlpha = fmod(abs(viewPos.y), step);
	fAlpha = 0.0;

    vec4 outColor = mix(vGrid1, vGrid2, fAlpha);

    vec3 vPseudoViewPos = vec3(viewPos.x, input.outWorldPos.y, viewPos.z);
    float fDistanceToCamera = max(distance(input.outWorldPos.xyz, vPseudoViewPos) - abs(viewPos.y), 0.0);
