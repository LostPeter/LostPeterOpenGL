...
OpenGL_008_Blend::OpenGL_008_Blend(int width, int height, String name)
	...
	, pShaderFragment_Outline(nullptr)
{
    ...
    this->shaderVertex_Path = getShaderPathRelative("standard_mesh_transparent.vert.spv", ShaderSort_Common);
    this->shaderFragment_Path = getShaderPathRelative("standard_mesh_transparent.frag.spv", ShaderSort_Common);
	this->shaderVertex_Path_Outline = getShaderPathRelative("standard_mesh_outline.vert.spv", ShaderSort_Common);
    this->shaderFragment_Path_Outline = getShaderPathRelative("standard_mesh_outline.frag.spv", ShaderSort_Common);
...
void OpenGL_008_Blend::createCamera()
{
...
void OpenGL_008_Blend::loadModel_Custom()
{
    for (int i = 0; i < g_CountLen; i++)
    {
        ...
        {
            String msg = "*********************** OpenGL_008_Blend::loadModel_Custom: Failed to load model: " + pModelObject->pathModel;
        ...
        {   
            String msg = "*********************** OpenGL_008_Blend::loadModel_Custom: Failed to load texture: " + pModelObject->pathTexture;
            F_LogError(msg.c_str());
            throw std::runtime_error(msg.c_str());
        }
...
bool OpenGL_008_Blend::loadModel_VertexIndex(ModelObject* pModelObject, bool isFlipY, bool isTransformLocal, const FMatrix4& matTransformLocal)
{
    //1> Load 
    FMeshData meshData;
    ...
    {
        F_LogError("*********************** OpenGL_008_Blend::loadModel_VertexIndex load model failed: [%s] !", pModelObject->pathModel.c_str());
        return false; 
    }
    ...
    F_LogInfo("OpenGL_008_Blend::loadModel_VertexIndex: load model [%s] success, Vertex count: [%d], Index count: [%d] !", 
		...
		pModelObject->poIndexBuffer_Data != nullptr)
	{
		...
		{
			F_LogError("*********************** OpenGL_008_Blend::loadModel_VertexIndex: create buffer vertex index failed: [%s] !", pModelObject->nameModel.c_str());
			return false;
		}
	}
	else
	{
		...
		{
			F_LogError("*********************** OpenGL_008_Blend::loadModel_VertexIndex: create buffer vertex failed: [%s] !", pModelObject->nameModel.c_str());
			return false;
		}
	}
...
bool OpenGL_008_Blend::loadModel_Texture(ModelObject* pModelObject)
{
    if (!pModelObject->pathTexture.empty())
    {
		...
		{
			F_LogError("*********************** OpenGL_008_Blend::loadModel_Texture: Failed to create texture, path: [%s] !", pModelObject->pathTexture.c_str());
			F_DELETE(this->poTexture)
			return false;
		}
        F_LogInfo("OpenGL_008_Blend::loadModel_Texture: Load texture [%s] success !", pModelObject->pathTexture.c_str());
    }

    return true;
}

void OpenGL_008_Blend::createCustomCB()
{
    ...
    for (size_t i = 0; i < count; i++)
    {
		...
		{
			String msg = "*********************** OpenGL_008_Blend::createCustomCB: create buffer uniform: [" + nameBuffer + "] failed !";
...
void OpenGL_008_Blend::createGraphicsPipeline_Custom()
{
    String namePathBase;
	...
	{
		String msg = "*********************** OpenGL_008_Blend::createGraphicsPipeline_Custom: Failed to create shader vertex: " + this->shaderVertex_Path;
		F_LogError(msg.c_str());
		throw std::runtime_error(msg);
	}
	...
	{
		String msg = "*********************** OpenGL_008_Blend::createGraphicsPipeline_Custom: Failed to create shader fragment: " + this->shaderFragment_Path;
		F_LogError(msg.c_str());
		throw std::runtime_error(msg);
	}
	...
	{
		String msg = "*********************** OpenGL_008_Blend::createGraphicsPipeline_Custom: Failed to create shader vertex: " + this->shaderVertex_Path_Outline;
		F_LogError(msg.c_str());
		throw std::runtime_error(msg);
	}
	...
	{
		String msg = "*********************** OpenGL_008_Blend::createGraphicsPipeline_Custom: Failed to create shader fragment: " + this->shaderFragment_Path_Outline;
		F_LogError(msg.c_str());
		throw std::runtime_error(msg);
	}
    ...
    for (size_t i = 0; i < count; i++)
    {
        ...
        {
            String msg = "*********************** OpenGL_008_Blend::createGraphicsPipeline_Custom: Failed to create pipeline stencil !";
            F_LogError(msg.c_str());
            throw std::runtime_error(msg.c_str());
        }
        ...
        {
            String msg = "*********************** OpenGL_008_Blend::createGraphicsPipeline_Custom: Failed to create pipeline outline !";
            F_LogError(msg.c_str());
            throw std::runtime_error(msg.c_str());
        }
...
void OpenGL_008_Blend::createDescriptorSets_Custom()
{
    size_t count = this->m_aModelObjects.size();
    for (size_t i = 0; i < count; i++)
    {
...
void OpenGL_008_Blend::updateCBs_Custom()
{
...
bool OpenGL_008_Blend::beginRenderImgui()
{
    ImGui_ImplOpenGL3_NewFrame();
    ImGui_ImplGlfw_NewFrame();
    ImGui::NewFrame();
    static bool windowOpened = true;
    ImGui::Begin("OpenGL_008_Blend", &windowOpened, 0);
    {
        //0> Common
        commonConfig();
...
void OpenGL_008_Blend::modelConfig()
{
...
void OpenGL_008_Blend::endRenderImgui()
{
    OpenGLWindow::endRenderImgui();

}

void OpenGL_008_Blend::drawMeshDefault_Custom()
{   
